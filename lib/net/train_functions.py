...
def model_joint_fn_decorator():
    ...
    def model_fn(model, data):
        if cfg.RPN.ENABLED:
            ...
            if not cfg.RPN.FIXED:
                rpn_cls_label, rpn_reg_label = data['rpn_cls_label'], data['rpn_reg_label'] #(B,N)(B,N,7)
        ...
        #print(ret_dict,ret_dict['gt_boxes3d'].view(-1,7).shape,ret_dict['rois'],ret_dict['roi_boxes3d'].shape)

        tb_dict = { }
        disp_dict = { }
        loss = 0
        if cfg.RPN.ENABLED and not cfg.RPN.FIXED:
            #print(ret_dict['rois'].shape,ret_dict['roi_boxes3d'].shape)
    ...
    def get_rpn_loss(model, rpn_cls, rpn_reg, rpn_cls_label, rpn_reg_label, tb_dict):
        ...
        # RPN regression loss
        point_num = rpn_reg.size(0) * rpn_reg.size(1) #rpn_reg(B,16384,76)
        fg_sum = fg_mask.long().sum().item()
        if fg_sum != 0:
            # for odiou3d loss 基数大很耗时
            # ret_box_decode = decode_bbox_target(rpn_reg_label.view(point_num, 7)[fg_mask],
            ...
            # loss_odious = odious_3d_loss(rpn_reg_label.view(point_num,7)[fg_mask], ret_box_decode, weights=None,
            #                              batch_size=fg_sum)
            # for reg loss
    ...
    def get_rcnn_loss(model, ret_dict, tb_dict):
        rcnn_cls, rcnn_reg = ret_dict['rcnn_cls'], ret_dict['rcnn_reg'] #(B,C)
        cls_label = ret_dict['cls_label'].float() #(B*64)，elem in [-1,0,1]-分类标签-101,用于分类权重
        reg_valid_mask = ret_dict['reg_valid_mask'] #(B*64)-iou_score>0.55的01
        roi_boxes3d = ret_dict['roi_boxes3d'] #(B*64,7),修正roi_size，基本无用
        roi_size = roi_boxes3d[:, 3:6]
        gt_boxes3d_ct = ret_dict['gt_of_rois'] #(B*64,7)-回归标签
        pts_input = ret_dict['pts_input']
        mask_score = ret_dict['mask_score'] #ce loss 基本无用
        gt_iou_weight = ret_dict['gt_iou'] #(B*64)-iou_score,无用
        ...
        elif cfg.RCNN.LOSS_CLS == 'BinaryCrossEntropy':#二分类
            rcnn_cls_flat = rcnn_cls.view(-1)
            batch_loss_cls = F.binary_cross_entropy(torch.sigmoid(rcnn_cls_flat), cls_label, reduction = 'none')
            cls_valid_mask = (cls_label_flat >= 0).float() #01
            rcnn_loss_cls = (batch_loss_cls * cls_valid_mask).sum() / torch.clamp(cls_valid_mask.sum(), min = 1.0)

        elif cfg.TRAIN.LOSS_CLS == 'CrossEntropy':#多分类
        ...
        fg_mask = (reg_valid_mask > 0) #大于0.55回归阈值
        fg_sum = fg_mask.long().sum().item()
        if fg_sum != 0:
            if cfg.USE_IOU_BRANCH:
            ...
            #for odiou3d loss
            ret_box_decode=decode_bbox_target(gt_boxes3d_ct.view(batch_size,7)[fg_mask],rcnn_reg.view(batch_size,-1)[fg_mask],
                                               ...
                                               get_ry_fine=True)
            odious_3d_loss=odious.odiou_3D()
            loss_odious=odious_3d_loss(gt_boxes3d_ct[fg_mask],ret_box_decode,weights=gt_iou_weight[fg_mask],batch_size=fg_sum)
            #for reg loss
            loss_loc, loss_angle, loss_size, loss_iou, reg_loss_dict = \
            ...
            if cfg.USE_IOU_BRANCH:
                ...
                #loss_odious = reg_loss_dict['loss_odious']
                rcnn_loss_reg = loss_loc + loss_angle + loss_size + loss_iou + loss_odious
