...
def model_joint_fn_decorator():
    ...
    def model_fn(model, data):
        ...
        if cfg.RPN.ENABLED and not cfg.RPN.FIXED:
            ...
            rpn_loss, rpn_loss_cls, rpn_loss_loc, rpn_loss_angle, rpn_loss_size, rpn_loss_iou = get_rpn_loss(model,
                                                                                                             ...
                                                                                                             rpn_reg_label,
                                                                                                             tb_dict)
            #rpn_loss_part = get_part_layer_loss(ret_dict)
            #rpn_loss+=rpn_loss_part
    ...
    def get_part_layer_loss(ret_dict,tb_dict=None):
        pos_mask = ret_dict['point_cls_labels'] > 0
        pos_normalizer = max(1, (pos_mask > 0).sum().item())
        point_part_labels = ret_dict['point_part_labels'] #[BN,3]
        point_part_preds = ret_dict['point_part_preds'] #[BN,3]
    ...
    def get_rcnn_loss(model, ret_dict, tb_dict):
        rcnn_cls, rcnn_reg = ret_dict['rcnn_cls'], ret_dict['rcnn_reg'] #(B*64)(B*64,46)
        cls_label = ret_dict['cls_label'].float() #(B*64)，elem in [-1,0,1]-分类标签,用于分类
        ...
        if fg_sum != 0:
            ...
            #for odiou3d loss
            # ret_box_decode=decode_bbox_target(roi_boxes3d.view(batch_size,7)[fg_mask],rcnn_reg.view(batch_size,-1)[fg_mask],
