...
#from pcn_coarse import AutoEncoder
from grnet_coarse import GRNet
...
class RCNNNet(nn.Module):
    def __init__(self, num_classes, input_channels=0, use_xyz=True):
        ...
        if cfg.RCNN.USE_RPN_FEATURES:
            ...
            self.xyz_up_layer = pt_utils.SharedMLP([self.rcnn_input_channel] + cfg.RCNN.XYZ_UP_LAYER,
                                                   bn=cfg.RCNN.USE_BN)
            self.xyz_up_layer1 = pt_utils.SharedMLP([3] + cfg.RCNN.XYZ_UP_LAYER,
                                                    bn=cfg.RCNN.USE_BN)
            c_out = cfg.RCNN.XYZ_UP_LAYER[-1]
            self.merge_down_layer = pt_utils.SharedMLP([c_out * 3, c_out], bn=cfg.RCNN.USE_BN)
    ...
    def forward(self, input_data):
        ...
        xyz, features = self._break_up_pc(pts_input)#(B*64,512,3)(B*64,130,512)

        if cfg.RCNN.USE_RPN_FEATURES: ## True
            # pcn补充点云
            #xyz_input = pts_input[..., 0:3].transpose(1, 2)  # (B*64,3,512)
            #self.pcn.eval()
            data={}
            data['partial_cloud']=pts_input[...,0:3].cpu().numpy() #(B*64,512,3)
            # 点云批量归一化
            ...
            pc = pc / m
            data['partial_cloud']=torch.from_numpy(pc).cuda()

            #　加载模型参数
            checkpoint = torch.load('/home/hqu/PycharmProjects/semester_project_cvlab-master2/EPNet-master/EPNet-master/model/GRNet-KITTI.pth')
            #print(checkpoint['grnet'])
            new_state_dict=OrderedDict()
            for k,v in checkpoint['grnet'].items():
                name=k[7:]
                new_state_dict[name]=v
            self.grnet.load_state_dict(new_state_dict)
            self.grnet.eval()
            with torch.no_grad():
                #_, xyz_coarse = self.pcn(xyz_input)
                xyz_coarse,xyz_dense = self.grnet(data)
            #print(xyz_coarse.shape,xyz_dense.shape)#(B*64,2048,3)(B*64,16384,3)
            xyz_coarse = xyz_coarse

            # 最远点采样
            new_xyz = pointnet2_utils.gather_operation(
                xyz_coarse.transpose(1,2).contiguous(),
                pointnet2_utils.furthest_point_sample(xyz_coarse.contiguous(), 512)
            ).contiguous()
            #print(new_xyz.shape) #(B*64,3,512)

            # 显示效果
            #for i in range(1):
            #    show_pcd(pts_input[...,0:3].cpu().numpy()[0,:,:]),show_pcd(new_xyz.transpose(1,2).cpu().numpy()[0,:,:])
            # 点特征
            new_xyz = new_xyz.unsqueeze(dim=3)
            pcn_feature = self.xyz_up_layer1(new_xyz)

            xyz_input = pts_input[..., 0:self.rcnn_input_channel].transpose(1, 2).unsqueeze(dim=3)  # (B*64,5,512,1)
            xyz_feature = self.xyz_up_layer(xyz_input)

            rpn_feature = pts_input[..., self.rcnn_input_channel:].transpose(1, 2).unsqueeze(dim=3)
            merged_feature = torch.cat((xyz_feature, rpn_feature, pcn_feature), dim=1)
            merged_feature = self.merge_down_layer(merged_feature)
            l_xyz, l_features = [xyz], [merged_feature.squeeze(dim=3)]  # (B*64,512,3)(B*64,128,512)
        else:
            l_xyz, l_features = [xyz], [features]#(B*64,512,3)(B*64,130,512)
