...
class RCNNNet(nn.Module):
    ...
    def forward(self, input_data):
        ...
        if cfg.RCNN.USE_RPN_FEATURES: ## True
            ...
            # 显示效果
            #for i in range(64):
            #    show_pcd(xyz_input.transpose(1,2).cpu().numpy()[i,:,:])#,show_pcd(xyz_coarse.transpose(1,2).cpu().numpy()[0,:,:])
            # 最远点采样
            new_xyz = pointnet2_utils.gather_operation(
                xyz_coarse,
                pointnet2_utils.furthest_point_sample(xyz_coarse.transpose(1, 2).contiguous(), 512)
            ).contiguous()
            # 点特征
            ...
            merged_feature = torch.cat((xyz_feature, pcn_feature, rpn_feature), dim=1)
        ...
        rcnn_reg = self.reg_layer(l_features[-1]).transpose(1, 2).contiguous().squeeze(dim=1)  # (B*64, 46)
        if cfg.USE_IOU_BRANCH:
            rcnn_iou_branch = self.iou_branch(l_features[-1]).transpose(1, 2).contiguous().squeeze(dim=1)  # (B*64,1)
