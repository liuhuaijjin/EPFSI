...
def boxes3d_to_bev_torch(boxes3d):
    ...
    cu, cv = boxes3d[:, 0], boxes3d[:, 2] #x,z
    half_l, half_w = boxes3d[:, 5] / 2, boxes3d[:, 4] / 2 #l/2,w/2
    boxes_bev[:, 0], boxes_bev[:, 1] = cu - half_l, cv - half_w #x-l/2,z-w/2
    boxes_bev[:, 2], boxes_bev[:, 3] = cu + half_l, cv + half_w #x+l/2,z+w/2
    boxes_bev[:, 4] = boxes3d[:, 6] #ry
    return boxes_bev
