...
## Transform the (cx, cz, w, l, theta) representation to 4 corners representation
    ...
    def forward(ctx, rbbox):
        '''
                    There is no rotation performed here. As axis are aligned.
                                     ...
                                     /          /    --->
                                    0 -------- 3     [x]
                    Each node has the coordinate of [x, z]. Corresponding the order of input.

                    Output: [N, 8]
                            [x_0, z_0, x_1, z_1, x_2, z_2, x_3, z_3],
        ...
        device = rbbox.device
        corners = torch.zeros((rbbox.shape[0], 8), dtype=torch.float32, device=device) #[N,8]
        dxcos = rbbox[:, 2].mul(torch.cos(rbbox[:, 4])) / 2.0 #w*cos(ry)/2
        dxsin = rbbox[:, 2].mul(torch.sin(rbbox[:, 4])) / 2.0 #w*sin(ry)/2
        dycos = rbbox[:, 3].mul(torch.cos(rbbox[:, 4])) / 2.0 #l*cos(ry)/2
        dysin = rbbox[:, 3].mul(torch.sin(rbbox[:, 4])) / 2.0 #l*sin(ry)/2
        corners[:, 0] = -dxcos - dysin + rbbox[:, 0] #-w*c/2-l*s/2+x
        corners[:, 1] = dxsin - dycos + rbbox[:, 1]  #w*s/2-lc/2+y
        corners[:, 2] = -dxcos + dysin + rbbox[:, 0] #-w*c/2+l*s/2+x
        corners[:, 3] = dxsin + dycos + rbbox[:, 1]  #w*s/2+l*c/2+y

        corners[:, 4] = dxcos + dysin + rbbox[:, 0]  #w*c/2+l*s/2+x
        corners[:, 5] = -dxsin + dycos + rbbox[:, 1] #-w*s/2+l*c/2+y
        corners[:, 6] = dxcos - dysin + rbbox[:, 0]  #w*c/2-l*s/2+x
        corners[:, 7] = -dxsin - dycos + rbbox[:, 1] #-w*s/2-l*c/2+y
        return corners
...
class odiou_3D(nn.Module):
    ...
    def forward(ctx, gboxes, qboxes, weights, batch_size):
        ...
        indicator = torch.gt(gboxes[:, 3], 0) & torch.gt(gboxes[:, 4], 0) & torch.gt(gboxes[:, 5], 0) \
                    & torch.gt(qboxes[:, 3], 0) & torch.gt(qboxes[:, 4], 0) & torch.gt(qboxes[:, 5], 0) #[True,Flase,True]
        index_loc = torch.nonzero(indicator) #[[0],[2]]
        ...
        corners_gboxes = rbbox_to_corners_object(gboxes[:, [0, 2, 3, 4, 6]]) #[x,z,w,l,ry]->[x1,y1,x2,y2,x3,y3,x4,y4]
        corners_qboxes = rbbox_to_corners_object(qboxes[:, [0, 2, 3, 4, 6]])
        corners_gboxes_1 = torch.stack((corners_gboxes[:, [0, 2, 4, 6]], corners_gboxes[:, [1, 3, 5, 7]]), 2) #(B,4,2)
        corners_qboxes_1 = torch.stack((corners_qboxes[:, [0, 2, 4, 6]], corners_qboxes[:, [1, 3, 5, 7]]), 2)
        corners_pts = torch.cat((corners_gboxes_1, corners_qboxes_1), 1) #(8,2)
        ...
        inter_h = (torch.min(gboxes[:, 1] + 0.5 * gboxes[:, 5], qboxes[:, 1] + 0.5 * qboxes[:, 5]) -
                   torch.max(gboxes[:, 1] - 0.5 * gboxes[:, 5], qboxes[:, 1] - 0.5 * qboxes[:, 5]))
        oniou_h = (torch.max(gboxes[:, 1] + 0.5 * gboxes[:, 5], qboxes[:, 1] + 0.5 * qboxes[:, 5]) -
                   torch.min(gboxes[:, 1] - 0.5 * gboxes[:, 5], qboxes[:, 1] - 0.5 * qboxes[:, 5]))
        inter_h[inter_h < 0] = 0

        #mbr_diag_3d_square = mbr_diag_bev**2 + inter_h ** 2 + 1e-7
        mbr_diag_3d_square = mbr_diag_bev ** 2 + oniou_h ** 2
        ...
        #ious_loss = 2.0 * batch_ious.sum() / batch_size #r=1.0
        return odious.mean()
...
#     x = torch.tensor([ 0.5, 0.5, 0.5, 1, 1, 1, 0], dtype=torch.float32).view(1, 7)
#     y = torch.tensor([ 1, 0, 0.5, 1, 1, 1, 0], dtype=torch.float32).view(1, 7)
#     print(odious_3d_loss(x, y, False,1))
#
