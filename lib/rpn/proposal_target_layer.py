...
class ProposalTargetLayer(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        sampled_pts, sampled_features = pooled_features[:, :, :, 0:3], pooled_features[:, :, :, 3:]#(B,64,512,3)(B,64,512,130)
        ...
        roi_center = batch_rois[:, :, 0:3] #(B,64,3)
        sampled_pts = sampled_pts - roi_center.unsqueeze(dim = 2)  # (B,64,512,3)
        ...
        reg_valid_mask = ((batch_roi_iou > cfg.RCNN.REG_FG_THRESH) & valid_mask).long() #01

        # classification label
        batch_cls_label = (batch_roi_iou > cfg.RCNN.CLS_FG_THRESH).long()#0.6
        invalid_mask = (batch_roi_iou > cfg.RCNN.CLS_BG_THRESH) & (batch_roi_iou < cfg.RCNN.CLS_FG_THRESH)#0.45 0.6
    ...
    def sample_rois_for_rcnn(self, roi_boxes3d, gt_boxes3d):
        ...
        :param gt_boxes3d: (B, M, 7) [x, y, z, h, w, l, ry]
        :return
            batch_rois: (B, 64, 7)
            batch_gt_of_rois: (B, 64, 7)
            batch_roi_iou: (B, 64)
        """
        batch_size = roi_boxes3d.size(0)

        fg_rois_per_image = int(np.round(cfg.RCNN.FG_RATIO * cfg.RCNN.ROI_PER_IMAGE)) #32=0.5*64
        ...
        for idx in range(batch_size):
            ...
            max_overlaps, gt_assignment = torch.max(iou3d, dim = 1) #维度最大值和索引
            #print(max_overlaps,gt_assignment)

            # sample fg, easy_bg, hard_bg
            fg_thresh = min(cfg.RCNN.REG_FG_THRESH, cfg.RCNN.CLS_FG_THRESH) #(0.55,0.6)
            ...
            easy_bg_inds = torch.nonzero((max_overlaps < cfg.RCNN.CLS_BG_THRESH_LO)).view(-1) #0.05
            hard_bg_inds = torch.nonzero((max_overlaps < cfg.RCNN.CLS_BG_THRESH) &
                                         (max_overlaps >= cfg.RCNN.CLS_BG_THRESH_LO)).view(-1) #0.05-0.45

            fg_num_rois = fg_inds.numel()
            bg_num_rois = hard_bg_inds.numel() + easy_bg_inds.numel()
            if fg_num_rois > 0 and bg_num_rois > 0:
                # sampling fg
                fg_rois_per_this_image = min(fg_rois_per_image, fg_num_rois) #(32,fg_num_rois)
                ...
                # sampling bg
                bg_rois_per_this_image = cfg.RCNN.ROI_PER_IMAGE #64
                bg_inds = self.sample_bg_inds(hard_bg_inds, easy_bg_inds, bg_rois_per_this_image)
            ...
            if bg_rois_per_this_image > 0:
                ...
                #print(bg_inds,iou3d_src)
                aug_times = 1 if cfg.RCNN.ROI_FG_AUG_TIMES > 0 else 0 #10
    ...
    def sample_bg_inds(self, hard_bg_inds, easy_bg_inds, bg_rois_per_this_image):
        if hard_bg_inds.numel() > 0 and easy_bg_inds.numel() > 0:
            hard_bg_rois_num = int(bg_rois_per_this_image * cfg.RCNN.HARD_BG_RATIO) #0.8
