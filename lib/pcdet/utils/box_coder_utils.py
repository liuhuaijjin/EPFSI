...
class PointResidualCoder(object):
    def __init__(self, code_size=8, use_mean_size=True, **kwargs):
        ...
        if self.use_mean_size:
            self.mean_size = torch.from_numpy(np.array(kwargs['mean_size'])).cuda().float() #[l,w,h]
    ...
    def encode_torch(self, gt_boxes, points, gt_classes=None):
        ...
        xg, yg, zg, dhg, dwg, dlg, rg = torch.split(gt_boxes, 1, dim=-1) #xyz,hwl,ry
        xa, ya, za = torch.split(points, 1, dim=-1)

        if self.use_mean_size:
            #assert gt_classes.max() <= self.mean_size.shape[0]
            #point_anchor_size = self.mean_size[gt_classes - 1]
            point_anchor_size = self.mean_size[0] #lwh
            dla, dwa, dha = torch.split(point_anchor_size, 1, dim=-1)
            diagonal = torch.sqrt(dla ** 2 + dwa ** 2)
            xt = (xg - xa) / diagonal
            zt = (zg - za) / diagonal
            yt = (yg - ya) / dha
        ...
        #cts = [g for g in cgs]
        return torch.cat([xt, yt, zt, dht, dwt, dlt, torch.sin(rg),torch.sin(rg)], dim=-1) #xyz,hwl,ry
    ...
    def decode_torch(self, box_encodings, points, pred_classes=None):
        ...
        """
        xt, yt, zt, dht, dwt, dlt, cost, sint= torch.split(box_encodings, 1, dim=-1) #xyz,hwl,ry
        xa, ya, za = torch.split(points, 1, dim=-1)

        if self.use_mean_size:
            #assert pred_classes.max() <= self.mean_size.shape[0]
            #point_anchor_size = self.mean_size[pred_classes - 1]
            point_anchor_size = self.mean_size[1-1] #lwh
            dla, dwa, dha = torch.split(point_anchor_size, 1, dim=-1)
            diagonal = torch.sqrt(dla ** 2 + dwa ** 2)
            xg = xt * diagonal + xa
            zg = zt * diagonal + za
            ...
            yg = yt + ya
            zg = zt + za
            dhg, dwg, dlg = torch.split(torch.exp(box_encodings[..., 3:6]), 1, dim=-1) #hwl

        rg = torch.atan2(sint, cost)

        #cgs = [t for t in cts]
        return torch.cat([xg, yg, zg, dhg, dwg, dlg, rg], dim=-1) #xyz,hwl,ry
